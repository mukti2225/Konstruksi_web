      ...
      <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 sm:py-10 lg:px-8">
        ...
        <div className="mb-6 flex flex-col gap-1 border-b border-[#E7E5E0] pb-5 sm:mb-10 sm:pb-6">
          <div className="flex flex-wrap items-end justify-between gap-2 sm:gap-3">
            <h1 className="text-2xl font-bold tracking-tight text-[#14181A] sm:text-3xl lg:text-4xl">Testimoni Klien</h1>
            <span className="font-mono text-xs text-[#6B7280] sm:text-sm">{loading ? "…" : `${items.length} testimoni terdaftar`}</span>
          </div>
        </div>
                ...
                <div className="flex items-center gap-1.5">
                  {[1, 2, 3, 4, 5].map((n) => (
                    <button key={n} type="button" onClick={() => setForm({ ...form, rating: n })} className="rounded-md p-1 transition hover:scale-110" aria-label={`Beri rating ${n}`}>
                      <Star className={`h-6 w-6 ${n <= form.rating ? "fill-green-400 text-green-400" : "fill-transparent text-[#D9D7D0]"}`} />
                    </button>
                  ))}
                  ...
                  <div key={item.id} className="group flex flex-col rounded-2xl border border-[#E7E5E0] bg-white p-5 shadow-[0_1px_2px_rgba(20,24,26,0.04)] transition hover:shadow-md">
                    <div className="mb-2 flex text-green-400">
                      {Array.from({ length: 5 }).map((_, i) => (
                        <Star key={i} className={`h-4 w-4 ${i < item.rating ? "fill-green-400 text-green-400" : "fill-transparent text-[#D9D7D0]"}`} />
                      ))}
                    </div>
