...
import { Plus, Pencil, Trash2, X, UploadCloud, Loader2, MapPin, ImageOff, Images } from "lucide-react";
...
export default function PortfolioAdminPage() {
  ...
  const [form, setForm] = useState({ title: "", location: "", image: "", images: [] as string[] });
  ...
  const resetForm = () => {
    setForm({ title: "", location: "", image: "", images: [] });
    setEditingId(null);
  };
  ...
  // Upload beberapa gambar sekaligus untuk galeri
  ...
  const handleEdit = (item: PortfolioItem) => {
    ...
    setForm({ title: item.title, location: item.location, image: item.image, images: item.images ?? [] });
  ...
  return (
    <div className="min-w-0">
      ...
      <div className="mb-6 flex flex-col border-b border-[#E7E5E0] pb-5 sm:mb-5 sm:pb-5">
        <h1 className="text-xl font-bold tracking-tight text-[#14181A] lg:text-2xl">Proyek Renovasi</h1>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[380px_1fr] lg:items-start lg:gap-8">
        <form onSubmit={handleSubmit} className="rounded-2xl border border-[#E7E5E0] bg-white p-5 shadow-[0_1px_2px_rgba(20,24,26,0.04)] sm:p-6 lg:sticky lg:top-8">
          ...
          <div className="space-y-4">
            ...
            {/* Cover Image */}
            <div>
              <label className="mb-1.5 block font-mono text-[11px] font-medium uppercase tracking-wider text-[#6B7280]">Foto Sampul (Cover)</label>
              {form.image ? (
                ...
                <label className="flex h-40 w-full cursor-pointer flex-col items-center justify-center gap-2 rounded-lg border-2 border-dashed border-[#D9D7D0] bg-[#FAFAF8] text-center transition hover:border-[#1E9E56] hover:bg-[#E6F5EC]/40 active:border-[#1E9E56]">
                  {uploading ? (
                    ...
                    <>
                      ...
                      <span className="text-xs text-[#6B7280]">Klik untuk unggah foto sampul</span>
            ...
            {/* Gallery Images */}
            <div>
              <label className="mb-1.5 block font-mono text-[11px] font-medium uppercase tracking-wider text-[#6B7280]">Galeri Foto Tambahan ({form.images.length})</label>

              {form.images.length > 0 && (
                <div className="mb-2 grid grid-cols-3 gap-2">
                  {form.images.map((url) => (
                    <div key={url} className="group relative h-20 w-full overflow-hidden rounded-lg border border-[#E7E5E0]">
                      <Image src={url} alt="Gallery" fill className="object-cover" />
                      <button
                        type="button"
                        onClick={() => removeGalleryImage(url)}
                        className="absolute right-1 top-1 flex h-5 w-5 items-center justify-center rounded-full bg-black/60 text-white opacity-100 transition sm:opacity-0 sm:group-hover:opacity-100"
                      >
                        <X className="h-3 w-3" />
                      </button>
                    </div>
              ...
              <label className="flex h-20 w-full cursor-pointer flex-col items-center justify-center gap-1 rounded-lg border-2 border-dashed border-[#D9D7D0] bg-[#FAFAF8] text-center transition hover:border-[#1E9E56] hover:bg-[#E6F5EC]/40">
                {galleryUploading ? (
                  <Loader2 className="h-4 w-4 animate-spin text-[#1E9E56]" />
                ) : (
                  <>
                    <Images className="h-4 w-4 text-[#9CA3AF]" />
                    <span className="text-[11px] text-[#6B7280]">Tambah foto galeri (bisa pilih banyak)</span>
                  </>
                )}
                <input type="file" accept="image/*" multiple className="hidden" onChange={(e) => e.target.files && e.target.files.length > 0 && handleGalleryUpload(e.target.files)} />
              </label>
            </div>
          </div>
          ...
          <div className="mt-6 flex gap-2">
            <button
              type="submit"
              disabled={uploading || galleryUploading || saving || !form.image}
        ...
        {/* List tidak berubah, cuma pastikan handleEdit ambil item.images */}
        <div>
          {loading ? (
            ...
            <div className="grid gap-4 sm:grid-cols-2 sm:gap-5 xl:grid-cols-2">
              {items.map((item, idx) => (
                <div key={item.id} className="group overflow-hidden rounded-2xl border border-[#E7E5E0] bg-white shadow-[0_1px_2px_rgba(20,24,26,0.04)] transition hover:shadow-md">
                  <div className="relative h-40 w-full overflow-hidden sm:h-44">
                    ...
                    {item.images?.length > 0 && <span className="absolute right-3 top-3 rounded-md bg-black/55 px-2 py-1 font-mono text-[11px] tracking-wide text-white backdrop-blur-sm">+{item.images.length} foto</span>}
                  </div>
                  <div className="p-4">
